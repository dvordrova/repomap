...
func expandGoSelection(
	...
	rootKey := goSelectionEntityKey(root)
	nodes[rootKey] = goSelectionNode{entity: root, distance: 0, root: true}
	frontier := []goSelectionFrontier{{entity: root, distance: 0}}
	...
	for len(frontier) > 0 && len(visited) < goSelectionMaxExactAnalyses {
		sort.Slice(frontier, func(i, j int) bool {
			return goSelectionFrontierLess(frontier[i], frontier[j], root, contentTerms)
		})
		current := frontier[0]
		frontier = frontier[1:]
