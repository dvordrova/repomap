...
func TestRepositoryTargetDispatchPreflightFailureFlushesFirstLayerSemanticJournal(t *testing.T) {
	...
	documentationProvider := &targetPortfolioClientStub{response: []byte(
		`{"overview":"Web application.","sources":[{"ref":"d1","claims":["Runs the TypeScript application."],"concepts":["web application"]}]}`,
	)}
	providerCalls := 0
	...
	if len(stages) != 3 || stages[debugdump.SemanticStageReadmeFileClassifier] != 1 ||
		stages[debugdump.SemanticStageTargetPortfolio] != 1 ||
		stages[debugdump.SemanticStageDocumentationReduce] != 1 {
