...
func TestRunEvaluatesSavedResponse(t *testing.T) {
	t.Parallel()
	...
	if err := os.WriteFile(bundlePath, deepseektest.SymbolBundleJSON, 0o600); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(responsePath, deepseektest.SymbolResponseJSON, 0o600); err != nil {
		t.Fatal(err)
	}
	...
	evaluationJSON, err := os.ReadFile(filepath.Join(outDir, "symbol_evaluation.json"))
	if err != nil {
		t.Fatal(err)
	}
	var evaluation symbol.Evaluation
