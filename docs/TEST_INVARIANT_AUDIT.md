...
| `internal/jstsproject/helper_test.go` | Owner-prepared local TypeScript compiler discovery binds source bytes and package ownership; neutral call patterns preserve exact external origins, selectors, path arguments, callback refs, and value provenance; package/bin/script identities, compiler aliases/fallback/ambiguity/exports, project references, workspace delegates, nested boundaries, exact selection, and credential scrubbing all remain closed and target-local. |
| `internal/jstsproject/model_test.go` | JS/TS sealed live facts and ProgramIndex projection are exact; reserved platform authority, product-surface and CLI seed rules, unresolved/dangling refs, optional sensitive metadata/signatures, credential-bearing manifest facts, and target identity all validate or fail closed. |
...
| `internal/report/program_index_projection_test.go` | Run restoration rebuilds the default portfolio/openable paths from exact ProgramIndex, DependencyCatalog, documentation, and GroupGraph artifacts, rejects mismatched set/snapshot/metadata, and never promotes invalid paths or package directories. |
| `internal/report/program_map_asset_test.go` | Rendered assets expose the final GroupGraph through the isolated three-lane Canvas, retain accessible interaction/source actions, omit retired presentation paths, and require exact persisted portfolio/graph authority. |
| `internal/report/program_page_portfolio_manifest_test.go` | Manifest verification binds the neutral page portfolio and requires target-outcome analyzed rows to form an exact bijection with validated pages. |
| `internal/report/program_portfolio_projection_test.go` | ProgramPortfolio keeps every exact target and one default, rejects duplicates, and accepts adapter-neutral languages without presentation registration. |
...
| `internal/report/report_test.go` | Report metadata and scale diagnostics retain complete authority across former local thresholds and preserve target-bound warning identity. |
...
| `internal/workspacesnapshot/snapshot_test.go` | Workspace snapshots build immutable source authority, require every allowed path to have captured input, and retain threshold-plus-one authority while reporting former size thresholds as diagnostic warnings. |
