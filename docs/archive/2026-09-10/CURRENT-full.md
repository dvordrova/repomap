...
Captured before the documentation split on 2026-09-11. This file preserves the
previous text, including superseded rules, experiments, measurements and known
contradictions. It is not required reading and does not authorize implementation.
Current decisions: [CURRENT](../../agent-room/CURRENT.md). Work routing: [AGENTS](../../../AGENTS.md).
...
Python adapter v15 follows one unconditional explicit imported binding through
indexed local package/module facades. It keeps the written import's module
boundary even when a subsequent call resolves the original declaration through
that facade. Written factory return annotations can then supply a possible
instance callback; calls and callback bindings remain `alternatives`. Multiple
bindings, conditional imports, deletions, wildcard imports, cycles and missing
members stop resolution. Dynamic export mutation and factory returns without
resolvable written types remain outside this source-only subset. No repository
module is executed. The cumulative Python example covers two facade layers,
renamed/module imports, reassignment contrasts and unrelated same-named classes;
the TypeScript barrel counterpart uses existing compiler resolution. Go has no
package-member re-export import statement to reproduce this Python failure.
The real Freqtrade native check resolves `freqtradebot.py:96` to the original
`ExchangeResolver.load_exchange` and the callback at line 174 to
`Exchange.ws_connection_reset` at `exchange.py:682`; this is native evidence,
not a new online operation-quality acceptance.

Go direct calls v11 / external calls v5, Python adapter v15 and JSTS helper v23
