...
- Every semantic request serializes the exact configured global output ceiling.
  ...
  The 2026-08-05 etcd run ended at 64,000 output tokens with
  `finish_reason=length`, but forensics proved a visible 56-ref generation
  repetition loop (6,390 package refs, 90 unique) — not capacity shortage —
  so the 64,000 ceiling is deliberately retained (Decision 215): a larger
  envelope would only extend the loop. Attempted Architecture output
  exhaustion is now isolated as an optional-stage failure (failed status +
  accounting, partial response diagnostic-only, local Canvas preserved)
  under Decision 215.
