...
Surface candidates are structural hints, not framework or runtime proof. The twelve-ref limit applies only to each entries[].family_refs list; it does not apply to surface_proposals. Examine every advertised surface candidate and return one proposal for every candidate whose exact facts establish a closed CLI-command or HTTP-route shape. Do not stop after twelve surface proposals. Return at most one proposal for an advertised candidate, and omit uncertain candidates. For a keyed_composite CLI command, use the advertised CLI kind ref, bind exactly one string fact to the identity slot, and optionally bind one callable fact to the handler slot. If callable facts exist but the handler is omitted, require another descriptive string fact establishing a parent command; one string setting paired with an unbound factory or helper callable is not a command. For a direct_call HTTP route, use the advertised HTTP kind ref and bind exactly one string or token fact to method, one string fact to path, and one callable fact to handler. A token method fact must case-insensitively equal CONNECT, DELETE, GET, HEAD, OPTIONS, PATCH, POST, PUT, or TRACE; a string method fact may preserve an exact custom HTTP method. Do not use other form, kind, slot, or fact combinations.
...
const PromptVersion = "entry-call-compression-prompt-827a9de5eb80"
