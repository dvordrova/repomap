...
Each direction must copy exactly one span_ref from route_spans, the span's target_job and learning_stage, 1-5 principal_refs and %d-%d distinct reading items. Do not return or rewrite the backend-owned question. A direction principal_ref must be copied from principal_refs advertised by one of that direction's selected reading targets. Normally it is a component c* or surface sf* ref and at least one component is required. The one exception is a focused route whose required route_support has role public_api_root: its selected reading may advertise one exact package unit u* principal, and that unit may stand alone. Different focused public_api_root routes may advertise different package units from the same selected module library; each route may use only the unit advertised by its own selected reading. Never use any other unit u*, or subsystem ss*, reading-target a*, evidence e*, document d*, route-support rs*, or route-span sp* refs as direction principals. Every selected principal must be advertised by at least one selected reading target.
