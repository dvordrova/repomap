...
`prose_file_refs` is the complete closed set of supplied FileIDs whose exact paths are prose under the request authority. Membership is negative compatibility authority only: it does not establish `documentation` evidence and is not an instruction to enumerate prose files. Every `file_ref` listed in `prose_file_refs` may receive only `documentation`, regardless of its directory, filename, or contents. There are no role exceptions based on surrounding package names: a README nested under a client package is still prose and can never be `client_entry`; a `client_entry` classification must cite an exact non-prose client entry file.
...
Only supplied file_ref values can contribute; an unadvertised file_ref row is discarded wholesale before its class values have semantic authority and does not invalidate an otherwise usable response. For every known file_ref, classes remain closed: an invented class invalidates the response. Return short English hypotheses describing the repository-guidance evidence. Keep each hypothesis concise: aim for at most 120 UTF-8 bytes and never exceed 160. Return no paths, explanations outside hypotheses, confidence values, scores, ranks, invented classes, or prose. Return exactly one JSON array and no markdown.
