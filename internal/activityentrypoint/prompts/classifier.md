...
The complete catalog is bounded locally by generic graph eligibility. For an executable target it contains exact callable launch seeds, callables without a fully exact incoming `calls`/`executes` relation, observed direct `calls`/`executes` targets of a launch seed, and every local callable participating in a `passes_callback`, `decorates`, or `implements` joint. For a library target it additionally contains every public callable. An exact `module` or `package` object is also advertised when a target launch seed identifies it as a module, main-guard, or script start. Such a seeded module/package row represents meaningful top-level execution for that target; unseeded modules and packages are not candidates. Eligibility deliberately preserves high-recall structural boundaries and carries no entrypoint authority. Names, signatures, visibility, paths, target seeds, and topology counts are evidence for your semantic decision; none proves an activity by itself. Do not use a hidden framework, protocol, TLS, dependency, filename, or naming allowlist.
...
`program_frontier` reports exact omissions already declared by the language adapter, indexed callables that have no source location, callables excluded by the declared generic eligibility boundary, and seeded module/package anchors that have no source location. `callables_ineligible` is intentional local bounding, not a hidden partial batch and not proof that excluded callables lack runtime activity. The advertised catalog is complete for eligible indexed activity objects with an exact source location, but those frontier counts mean you must not infer that the index proves the absence of other runtime activity. Do not repair or invent the missing facts.
