...
func (kind PlaceKind) Valid() bool {
	switch kind {
	case PlaceDirectory, PlaceFile, PlaceSymbol, PlaceBoundary:
		return true
	default:
		return false
	}
...
type Decl struct {
	...
	// Doc is the first sentence of the author's docstring, when there is one.
	Doc      string `json:"doc,omitempty"`
...
func EncodeGraph(graph Graph) ([]byte, error) {
	...
	digest, err := digestOf("repomap-atlas-graph-v1\x00", graph)
	if err != nil {
		return nil, err
	}
	graph.SHA256 = digest
...
func Encode(value Atlas) ([]byte, error) {
	...
	digest, err := digestOf("repomap-atlas-v1\x00", value)
	if err != nil {
		return nil, err
	}
	value.SHA256 = digest
