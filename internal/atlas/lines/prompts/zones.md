...
- `names`: there is one row: the target's largest boxes, each with its
  ...
  is a part on its own. Name what a part does, not where it is.
...
`key` values as the request, each exactly once, and only the cell the
question asks for:

```json
{
  "rows": [
    {"key": "r1", "part": "Report rendering"},
    {"key": "r2", "part": "Report rendering"},
    {"key": "r3", "part": "Program indexing"}
  ]
}
```
