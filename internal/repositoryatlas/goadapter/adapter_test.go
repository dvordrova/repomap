...
func TestProjectKeepsExactProcessIdentityIndependentOfTraceAvailability(t *testing.T) {
	availabilities := []string{
		"",
		surfacediscovery.AvailabilityUnknown,
		surfacediscovery.AvailabilityUnavailable,
		surfacediscovery.AvailabilityAvailable,
	}
	roles := []string{
		"",
		surfacediscovery.ExecutableRolePrimaryApplication,
		surfacediscovery.ExecutableRoleSecondaryService,
		surfacediscovery.ExecutableRoleTooling,
		surfacediscovery.ExecutableRoleTestOrHelper,
		surfacediscovery.ExecutableRoleUnknown,
	}
	for _, availability := range availabilities {
		for _, role := range roles {
			availability, role := availability, role
			t.Run("availability="+availability+"/role="+role, func(t *testing.T) {
				input := singleAppInput(
					"fixture", "module-fixture", "example.com/fixture", ".",
					"example.com/fixture/cmd/app", "cmd/app", "cmd/app/main.go", 7, "trigger-app",
				)
				input.Catalog.Triggers[0].Availability = availability
				input.Catalog.Triggers[0].ExecutableRole = role
				atlas, err := Project(input)
				if err != nil {
					t.Fatal(err)
				}
				if len(atlas.Entities) != 2 || len(atlas.Evidence) != 1 ||
					len(atlas.Observations) != 2 || len(atlas.Relations) != 1 {
					t.Fatalf("exact process identity was lost with availability %q role %q: %#v", availability, role, atlas)
				}
			})
		}
	}
