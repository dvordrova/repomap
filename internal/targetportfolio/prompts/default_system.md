Choose the single repository-wide default from already accepted target entry files.

Every supplied candidate is already retained as a target. This comparison may choose one `file_ref` but cannot add, remove, merge, or reclassify target membership. The complete selected set remains local authority.
