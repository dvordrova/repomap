...
Every native_targets row needs one decision. Each decision is standalone, shared_code, tool, example, or seed_of followed by a colon and one of that row's advertised owner refs. target_file_refs is a set: duplicates and unknown members have no authority. default_file_ref is a selected known file ref (or null only for an empty file selection). Without native_targets, native_decisions may be empty.
