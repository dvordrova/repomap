...
func TestOwnerAttemptTimeoutSplitsAndWarmRunSkipsExpiredParent(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		client := llmProviderHandlerClient(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			if strings.Contains(request.Messages[1].Content, "a,b") {
				<-r.Context().Done()
				w.WriteHeader(500) // the endpoint expires the long generation
				return
			}
			_, _ = w.Write(llmProviderResponse("stop", `{"ok":true}`, nil))
		}))
		client.OnRetry = func(RetryProgress) { t.Error("expired owner attempt retried identical bytes") }
		...
		if elapsed := time.Since(started); elapsed != 4*time.Minute {
			t.Fatalf("expired request consumed %v, want one four-minute attempt", elapsed)
		}
	})
}
