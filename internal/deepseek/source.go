...
const SourcePromptVersionJSON = "source-assessment-json-v5"
...
2. Use "shown" only when the cited source lines directly show the anchored source operation. A shown assessment must cite its anchor_source_evidence_id. For checks_call_result, when a call result is used by a separate guard or returned comparison, cite both the call anchor and that result-use line; the anchor alone is ambiguous. For returns_call_result, cite the exact direct-return call anchor. For calls_from_branch, cite both the locally visible branch line and call anchor; either line alone is ambiguous.
...
5. Match the predicate to written syntax, not the callee name alone. validates_input requires a guard, conditional, or returned nil comparison using the call result. maps_error requires returning the mapped result on a shown error path. delegates_operation supports only that the call is made at the operation step. fills_response requires response data in the shown call. checks_call_result supports only that written source immediately checks or compares a value produced by the call. returns_call_result requires the call itself to be the direct return expression, without a wrapper or transform. calls_from_branch requires a complete call statement under a locally visible case, if, or else branch; it does not prove which branch runs. Persistence and I/O names do not prove runtime side effects.
