	...
	for _, place := range graph.Places {
		...
		for _, call := range wire.Calls {
			...
			for i, argument := range call.SourceArguments {
				...
				v := argument.Origin
				if argument.Position != i+1 || v == nil || v.Kind != "field" || v.Text != want || v.Anchor == nil || v.Anchor.Path != path || v.Anchor.Line < 1 || v.Anchor.Column < 1 || len(v.Parts) != 1 || v.Parts[0].Text != "app" {
