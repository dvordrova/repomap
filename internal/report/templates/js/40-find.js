...
(function () {
  ...
  box.placeholder='Find a question, term, part, operation or code';box.setAttribute('aria-label','Find in repository');
  var panel=document.createElement('section');panel.className='find-panel';panel.hidden=true;panel.id='repository-find';
  panel.setAttribute('aria-label','Repository search results');box.setAttribute('aria-controls',panel.id);
  panel.innerHTML='<div class="find-tools"><label>Show <select aria-label="Search result type"><option value="all">Everything</option><option value="question">Questions and answers</option><option value="term">Terms</option><option value="part">Parts</option><option value="operation">Operations</option><option value="code">Code</option></select></label><label>In <select aria-label="Search component"><option value="">All components</option></select></label><button type="button" data-close>Close</button></div><p class="find-status" role="status"></p><ol class="find-results"></ol><div class="find-pages"><button type="button" data-prev>← Previous</button><span></span><button type="button" data-next>Next →</button></div>';
  ...
  document.querySelectorAll('.repo-map .repo-node[data-node]').forEach(function(n){
    ...
    add({title:n.dataset.title,summary:n.dataset.summary,component:n.dataset.title,section:id,kind:'part',type:'Component',destination:document.getElementById(id)});
  });
  ...
  function sourceCard(node,selector){
    ...
    appendText(card,'strong','Model response');
    ...
    appendText(card,'p',sources.querySelector('a')?'The model cited these sources:':'No citations were saved for this text.');
  ...
  document.querySelectorAll('.reading-guide').forEach(function(n){
    ...
    add({title:n.querySelector('.reading-question').textContent,summary:summary,component:'Repository questions',section:'questions',sections:sectionsFor(n,'[data-question-map]'),kind:'question',type:'Question',destination:n,sourceCard:sourceCard(n,'.answer-copy')});
  });
  document.querySelectorAll('.learn-concept').forEach(function(n){
    add({title:n.querySelector('summary').textContent,summary:modelText(n.querySelector('.model')),component:'Term explanation',section:'concepts',sections:sectionsFor(n,'a[href^="#"]'),kind:'term',type:'Term',destination:n,sourceCard:sourceCard(n,':scope')});
  });
  ...
  function description(entry,li,terms){
    ...
    if(!entry.sourceCard){var card=document.createElement('aside');card.className='source-card';appendText(card,'strong','Model response');appendText(card,'p','This is the description used on the map. No separate citations were saved for this text.');entry.sourceCard=card;}
    var hint=document.createElement('button');hint.type='button';hint.className='source-hint';hint.textContent='ⓘ';hint.setAttribute('aria-label','About this description');text.appendChild(hint);repomapPreview.bind(text,entry.sourceCard);
  }
  function render(){
    var q=box.value.trim().toLowerCase();if(q!==lastQuery){page=0;lastQuery=q;}
    if(!q){close();return;}
    var terms=q.split(/\s+/),matches=entries.filter(function(e){return (kind.value==='all'||e.kind===kind.value)&&(!component.value||e.section===component.value||e.sections?.includes(component.value))&&terms.every(function(t){return e.haystack.includes(t);});});
    matches.sort(function(a,b){return rank(a,q)-rank(b,q)||a.title.localeCompare(b.title)||a.component.localeCompare(b.component);});
    page=Math.min(page,Math.max(0,Math.ceil(matches.length/pageSize)-1));
    panel.hidden=false;box.setAttribute('aria-expanded','true');results.replaceChildren();
    status.textContent=matches.length?matches.length+' results · choose one to open its answer, explanation, map or source':'No matching question, explanation, part, operation or code. Try a shorter name or another word.';
    matches.slice(page*pageSize,(page+1)*pageSize).forEach(function(e){
      ...
      if(e.kind==='code'){
        ...
        e.memberships.forEach(function(m){links.appendChild(action('In '+m.title+' →',membership(e,m)));});
        if(e.source.tagName==='A'){var source=e.source.cloneNode(false);source.className='find-source';source.textContent='Open code ↗';links.appendChild(source);}
        li.appendChild(links);
      }
      results.appendChild(li);
    });
    results.scrollTop=0;
    pages.hidden=matches.length<=pageSize;pages.querySelector('span').textContent=(page*pageSize+1)+'–'+Math.min((page+1)*pageSize,matches.length)+' of '+matches.length;
  ...
  panel.querySelector('[data-close]').addEventListener('click',function(){close();box.focus();close();});
  ...
  nav.addEventListener('keydown',function(e){if(e.key==='Escape'){close();box.focus();close();}if(e.target===box&&(e.key==='Enter'||e.key==='ArrowDown')){var first=results.querySelector('button');if(first&&!panel.hidden){e.preventDefault();first.focus();}}});
  document.addEventListener('keydown',function(e){if(e.key==='/'&&!e.ctrlKey&&!e.metaKey&&!e.altKey&&!/^(INPUT|TEXTAREA|SELECT)$/.test(document.activeElement.tagName)&&!document.activeElement.isContentEditable){e.preventDefault();box.scrollIntoView({block:'center'});box.focus();}});
