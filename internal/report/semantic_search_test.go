...
func TestSemanticSearchDisabledOmitsOnlyTheSearchProjectionAndAssets(t *testing.T) {
	defaultData := semanticSearchTestReport()
	defaultHTML, err := RenderHTML(defaultData)
	if err != nil {
		t.Fatal(err)
	}
	for _, marker := range []string{
		...
		if !strings.Contains(string(defaultHTML), marker) {
			t.Fatalf("default report is missing semantic search marker %q", marker)
		}
	}

	disabledData := semanticSearchTestReport()
	originalGuidedTour := disabledData.GuidedTour
	originalSemanticArtifacts := append([]semanticdiscovery.Artifact(nil), disabledData.SemanticArtifacts...)
	staleIndex, err := BuildSemanticSearchIndex(disabledData)
	if err != nil {
		t.Fatal(err)
	}
	disabledData.SemanticSearch = &staleIndex
	disabledData.SemanticSearchDisabled = true
	if err := attachSemanticSearchIndex(disabledData); err != nil {
		t.Fatal(err)
	}
	if disabledData.SemanticSearch != nil {
		t.Fatal("disabled report retained a semantic search index")
	}
	if !reflect.DeepEqual(disabledData.GuidedTour, originalGuidedTour) {
		t.Fatal("disabling search changed the guided semantic artifact")
	}
	if !reflect.DeepEqual(disabledData.SemanticArtifacts, originalSemanticArtifacts) {
		t.Fatal("disabling search changed semantic discovery artifacts")
	}

	// A stale saved index must not bypass the render-time policy gate.
	disabledData.SemanticSearch = &staleIndex
	disabledHTML, err := RenderHTML(disabledData)
	if err != nil {
		t.Fatal(err)
	}
	for _, marker := range []string{
		...
		if strings.Contains(string(disabledHTML), marker) {
			t.Fatalf("disabled report unexpectedly contains semantic search marker %q", marker)
		}
	}
	for _, marker := range []string{`"semantic_search_disabled":true`, `"guided_tour":`, `"semantic_artifacts":`} {
		if !strings.Contains(string(disabledHTML), marker) {
			t.Fatalf("disabled report is missing retained report marker %q", marker)
		}
	}
}

func TestSemanticSearchPolicyReplaysFromRunMetadata(t *testing.T) {
	tests := []struct {
		name            string
		metadata        string
		wantDisabled    bool
		wantSearch      bool
		regenerateTwice bool
	}{
		{name: "default remains enabled", metadata: `{}`, wantSearch: true},
		{
			name:            "no search remains disabled",
			metadata:        `{"effective_options":{"no_search":true}}`,
			wantDisabled:    true,
			regenerateTwice: true,
		},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			dir := t.TempDir()
			writeTestFile(t, dir, "snapshot.json", `{"repo_name":"search-policy"}`)
			writeTestFile(t, dir, "metadata.json", test.metadata)
			writeTestFile(t, dir, "orientation_report.json", `{"project_guess":"fixture","candidate_flows":[],"warnings":[]}`)

			data, err := ReadRunDir(dir)
			if err != nil {
				t.Fatal(err)
			}
			if data.SemanticSearchDisabled != test.wantDisabled {
				t.Fatalf("SemanticSearchDisabled = %t, want %t", data.SemanticSearchDisabled, test.wantDisabled)
			}
			if (data.SemanticSearch != nil) != test.wantSearch {
				t.Fatalf("SemanticSearch present = %t, want %t", data.SemanticSearch != nil, test.wantSearch)
			}

			generateCount := 1
			if test.regenerateTwice {
				generateCount = 2
			}
			for index := 0; index < generateCount; index++ {
				if err := Generate(dir); err != nil {
					t.Fatalf("Generate() pass %d: %v", index+1, err)
				}
			}
			...
			var saved ReportData
			if err := json.Unmarshal(reportJSON, &saved); err != nil {
				t.Fatal(err)
			}
			if saved.SemanticSearchDisabled != test.wantDisabled || (saved.SemanticSearch != nil) != test.wantSearch {
				t.Fatalf(
					"saved policy disabled=%t search=%t, want disabled=%t search=%t",
					saved.SemanticSearchDisabled,
					saved.SemanticSearch != nil,
					test.wantDisabled,
					test.wantSearch,
				)
			}
