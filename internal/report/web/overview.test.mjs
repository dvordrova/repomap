...
test('overview folds only saved areas and preserves every part, input, external identity and directed relation',async()=>{
  const items=[
    ...
    {id:'a',title:'Input handling',height:180,inputs:[{id:'input',title:'Submit',height:58}]},
  ...
  assert.equal(result.summaries.get('ui').members[0].inputs[0].id,'input');
  ...
  assert.equal(result.layout.edges.length,4,'all original relations survive beneath the summary');
