# D216 draft — bounded local Architecture units before semantic grouping

Status: proposal only. Do not implement before D215 is accepted.
...
- 635 relations, including 613 package imports.

The model began one very broad component and entered an exact 56-package-ref
cycle until the 64k output ceiling. The current flat member task is not robust
for a large modular server.
